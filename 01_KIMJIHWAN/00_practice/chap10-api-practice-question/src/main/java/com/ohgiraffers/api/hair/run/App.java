...
public class App {
    ...
    * */
    public static void main(String[] args) {
        ...
        User[] users = new User[token.countTokens()];
        int i = 0;

        while(token.hasMoreTokens()) {
            String[] ud = token.nextToken().split(",");

            User u = new User();

            u.setId(Integer.parseInt(ud[0]));
            u.setFirstName(ud[1]);
            u.setLastName(ud[2]);
            u.setEmail(ud[3]);
            u.setGender(ud[4]);

            if (ud[5] != null && !ud[5].isBlank()) {
                String cleaned = ud[5].trim().replace("/", "-");  // 슬래시 → 하이픈 변환
                LocalDate birth = LocalDate.parse(cleaned, DateTimeFormatter.ofPattern("yyyy-MM-dd"));
                u.setBirthday(birth);
            }
                LocalDate ld = LocalDate.parse(ud[6], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
                u.setReservationDate(ld);

            if (ud[7] != null && !ud[7].isBlank()) {
                String cleaned = ud[7].trim();
                LocalTime lt = LocalTime.parse(cleaned, DateTimeFormatter.ofPattern("H:mm"));  // 0~23 한 자리 대응
                u.setReservationTime(lt);
            }

            LocalDateTime ldt = LocalDateTime.parse(ud[8]);
                u.setCreatedAt(ldt);
...
//        User[] users = new User[temp.length];
//        for(int i = 0; i < temp.length; i++) {
//            if(i == 0) continue;
//            String[] each = temp[i].split(",");
//            long id = Long.parseLong(each[0]);
//            String firstName = each[1];
//            String lastName = each[2];
//            String email = each[3];
//            String gender = each[4];
//            LocalDate birthday = !"".equals(each[5]) ?
//                    LocalDate.parse(each[5], DateTimeFormatter.ofPattern("yyyy/MM/dd")) :
//                    null;
//            LocalDate reservationDate = LocalDate.parse(each[6]);
//            LocalTime reservationTime = LocalTime.parse(each[7], DateTimeFormatter.ofPattern("H:m"));
//            LocalDateTime createdAt = LocalDateTime.parse(each[8]);
//            users[i] = new User(id, firstName, lastName, email, gender, birthday, reservationDate, reservationTime, createdAt);
